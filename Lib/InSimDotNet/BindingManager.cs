...
namespace InSimDotNet
{
    ...
    internal sealed class BindingManager
    {
        ...
        private void BindInternal(PacketType packetType, IPacketBinding binding)
        {
            ...
            {
                if (packetBindings.TryGetValue(packetType, out List<IPacketBinding> bindings))
                {
        ...
        {
            if (packetBindings.TryGetValue(packet.Type, out List<IPacketBinding> bindings))
            {
        ...
        {
            private readonly PacketHandler callback;
            ...
            {
                return obj is PacketBinding binding && binding.callback == callback;
            }
        ...
        private class PacketBinding<TPacket> : IPacketBinding
            where TPacket : IPacket
        {
            private readonly PacketHandler<TPacket> callback;
            ...
            public override bool Equals(object obj)
            {
                return obj is PacketBinding<TPacket> binding && binding.callback == callback;
            }
