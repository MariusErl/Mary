...
namespace Mary
{
    class Pitstop
    {
        static public void PitStop(InSim insim, IS_PIT pit)
        {
            var user = Program.GetClient(pit.PLID, false);
            if (user == Program.MySelf)
            {
        ...
        static public void PitFinished(InSim insim, IS_PSF psf)
        {
            try
            {
                ...
                if (user == Program.MySelf && PITButton.Enabled)
                {
                    PITButton.Dispose();
